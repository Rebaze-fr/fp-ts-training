...
) => Task<Option<Currency>> = flow(option.map(getCountryCurrency), x => x, option.traverse(task.ApplicativePar)(x => x));
...
export const giveCurrencyOfCountryToUser: (
  countryNameFromUserMock: string,
) => Task<Option<Currency>> = countryNameFromUserMock => pipe(
  getCountryNameFromUser(countryNameFromUserMock),
  task.map(getCountryCode),
  task.chain(option.traverse(task.ApplicativePar)(getCountryCurrency)),
);
